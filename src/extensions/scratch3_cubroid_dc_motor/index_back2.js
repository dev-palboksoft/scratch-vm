...
const BLEUUID = {
    name: 'DC Motor',
    service: 'B7C23878-E520-466E-9B5B-480407AB4870',
    characteristic: 'B7C23878-E520-466E-9B5B-480407AB4871'
};
...
class CubroidDcMotor {
    ...
    send (command) {
        ...
        // let msg = Buffer.from('0xFF00', 'hex').toString('base64')
    
        let msg = [
            command
        ];
        msg.unshift(msg.length + 1);
        let masses = Base64Util.uint8ArrayToBase64(msg)

        this._ble.write(BLEUUID.service, BLEUUID.characteristic, masses, null, false).then(
            () => {
                this._busy = false;
...
/**
 * Enum for cubroid db motor states.
 * @readonly
 * @enum {string}
 */
const MotorOptions = {
...
class Scratch3CubroidDcMotorBlocks {
        ...
        return [
            {
                ...
                value: MotorOptions.stop.command
            },
            {
                text: formatMessage({
                    id: 'cubroiddcmotor.motoroptionmenu.go',
                    default: MotorOptions.go.name,
                    description: 'label for moved gesture in gesture picker for micro:bit extension'
                }),
                value: MotorOptions.go.command
            },
            {
                text: formatMessage({
                    id: 'cubroiddcmotor.motoroptionmenu.back',
                    default: MotorOptions.back.name,
                    description: 'label for moved gesture in gesture picker for micro:bit extension'
                }),
                value: MotorOptions.back.command
            }
        ];
    }
            ...
            blocks: [
                {
                    opcode: 'motorControl',
                    text: formatMessage({
                        id: 'cubroiddcmotor.motorControl',
                        default: 'Motor 1 [ACTION]',
                        description: 'Motor 1'
                    }),
                    ...
                    arguments: {
                        ACTION: {
                            ...
                            menu: 'motorAction',
                            defaultValue: MotorOptions.go.command
                        }
                    }
                }
            ],
    ...
    motorControl (args){
      // console.log(this._peripheral.isConnected());
      this._peripheral.send(args.ACTION);
      return args.OPT;
    }
}
